...
<body>
	...
	<c:choose>
		...
		<c:set var="message1" scope="request"  value="${invalidParameter }" /> 
		 <c:if test="${not empty message1 }">
			<div>${message1 }</div>
		 </c:if> 
		<c:set var="message2" scope="request" value="${badNewPrice }" />
		 <c:if test="${not empty message2 }">
			...
			<table>
				...
				<c:forEach items="${ActiveEnglandLots }" var="lot">
					<tr>
						<td>${lot.lotName }</td>
						<fmt:parseDate value="${lot.startSellingDate}"
							var="parsedCurrentDate" pattern="yyyy-MM-dd'T'HH:mm" />
						<td><fmt:formatDate type="both" pattern="HH:mm dd MMMM yyyy"
								value="${parsedCurrentDate}" /></td>
						<td><fmt:formatNumber value="${lot.getEndPrice(locale)}"
